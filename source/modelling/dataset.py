...
from const import IMAGES, TARGETS
...
class PosterDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> tp.Dict[str, np.ndarray]:
        ...
        img_path = row.path
        target = np.array(row.drop(['path']), dtype='float32')
...
def get_class_names(config: Config) -> tp.List[str]:
    class_names = list(pd.read_csv(config.train_dataset_path, nrows=1).drop('path', axis=1))
...
def get_loaders(config: Config) -> tp.Tuple[tp.OrderedDict[str, DataLoader], tp.Dict[str, DataLoader]]:  # noqa: WPS210
